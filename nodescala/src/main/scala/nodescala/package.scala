...
package object nodescala {
  ...
  implicit class FutureCompanionOps(val f: Future.type) extends AnyVal {
    ...
    def all[T](fs: List[Future[T]]): Future[List[T]] = {
      ...
      if (fs.isEmpty)
        ...
        fs.head.onComplete {
          case Success(value) =>
            all(fs.tail).onComplete {
              case Success(tailValue) => p.success(value :: tailValue)
              case Failure(exception) => p.failure(exception)
            }
          case Failure(exception) =>
            all(fs.tail).onComplete(_ => p.failure(exception))
        }
      p.future
    }
    ...
    def any[T](fs: List[Future[T]]): Future[T] = {
      val p = Promise[T]()
      fs.foreach(f => f.onComplete(tryValue =>
    ...
    def delay(t: Duration): Future[Unit] = {
      ...
      Future {
        try {
          ...
          case _: TimeoutException => p.success(())
        }
      }
      p.future
    }
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
    ...
    def continue[S](cont: Try[T] => S): Future[S] = {
      val p = Promise[S]()
      f.onComplete(tryValue => p.success(cont(tryValue)))
      p.future
    }
